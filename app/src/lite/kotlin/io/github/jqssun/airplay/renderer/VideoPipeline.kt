...
class VideoPipeline {
    ...
    private val positionBuffer = _fb(POSITIONS)
    private val textureBuffer = _fb(TEXTURE_NORMAL)
    ...
    private fun _loop() {
        ...
        while (true) {
            ...
            var doRedraw = false
            synchronized(lock) {
                while (running && !frameAvailable && !displayDirty && !redrawRequested) lock.wait()
            ...
            else if (doRedraw && hasFrame && window != EGL14.EGL_NO_SURFACE) _render()
        }
        _releaseGl()
    }
    ...
    private fun _updateGeometry() {
        if (!geometryDirty) return
        val layout = VideoLayoutCalculator.calculate(videoW, videoH, winW, winH)
        ...
        val texture = if (layout.rotateClockwise) TEXTURE_ROTATE_CLOCKWISE else TEXTURE_NORMAL
        positionBuffer.clear(); positionBuffer.put(positions); positionBuffer.position(0)
        textureBuffer.clear(); textureBuffer.put(texture); textureBuffer.position(0)
        geometryDirty = false
        Log.i(
            TAG,
            "Video layout ${videoW}x${videoH} -> ${winW}x${winH}, " +
                "scale=${layout.scaleX}x${layout.scaleY}, rotate=${layout.rotateClockwise}",
        )
    }
    ...
    companion object {
        ...
        private val TEXTURE_NORMAL = floatArrayOf(0f, 0f, 1f, 0f, 0f, 1f, 1f, 1f)
        private val TEXTURE_ROTATE_CLOCKWISE = floatArrayOf(0f, 1f, 0f, 0f, 1f, 1f, 1f, 0f)
