...
class VideoRenderer {
    ...
    fun setResolution(w: Int, h: Int) = synchronized(lock) {
        if (w <= 0 || h <= 0 || (w == videoWidth && h == videoHeight)) return@synchronized
        val formatChanged = videoWidth > 0 && videoHeight > 0
        videoWidth = w
        videoHeight = h
        pipeline.setVideoSize(w, h)
        if (formatChanged && codec != null) {
            // iOS changes dimensions when its orientation changes. API 21 decoders are not
            // consistently adaptive, so restart cleanly and wait for the next keyframe.
            stopCodec()
        }
    }
    ...
    private fun startCodec(h265: Boolean) {
        ...
        if (enforceSdr && android.os.Build.VERSION.SDK_INT >= 24) {
            format.setInteger(MediaFormat.KEY_COLOR_STANDARD, MediaFormat.COLOR_STANDARD_BT709)
            format.setInteger(MediaFormat.KEY_COLOR_RANGE, MediaFormat.COLOR_RANGE_LIMITED)
            format.setInteger(MediaFormat.KEY_COLOR_TRANSFER, MediaFormat.COLOR_TRANSFER_SDR_VIDEO)
        }
        if (realtimeDecoderPriority && android.os.Build.VERSION.SDK_INT >= 23) {
    ...
    /**
     * Ends only the current iPhone video session. The EGL pipeline and display
     * surface remain reusable, avoiding a costly receiver/service restart when
     * iOS switches from Waze mirroring to an audio application.
     */
    fun resetStream() = synchronized(lock) {
        stopCodec()
        videoWidth = 0
        videoHeight = 0
        currentH265 = false
