...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit(
            "usage: verify_android_1_8_19_webview_ui.py REFERENCE_1816 REBUILT_1819"
        )
    ...
    for marker in (
        ...
        b"setOnClickListener", b"about:blank", b"background:transparent",
    ):
    ...
    if "versionCode: 10819" not in config or "versionName: 1.8.19" not in config:
        raise RuntimeError("rebuilt APK does not report 1.8.19")
