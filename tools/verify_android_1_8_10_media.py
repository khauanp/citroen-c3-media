#!/usr/bin/env python3
"""Verify 1.8.10 isolates media churn and freezes the accepted 1.8.8 map."""
...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit("usage: verify_android_1_8_10_media.py REFERENCE_188 REBUILT_1810")
    ...
    for path in sorted(map_paths):
        if old.get(path) != new.get(path):
            raise RuntimeError(f"accepted 1.8.8 map changed in 1.8.10: {path}")

    changed = {path for path in old.keys() & new.keys() if old[path] != new[path]}
    added = set(new) - set(old)
    deleted = set(old) - set(new)
    allowed = {SERVICE, AUDIO, HOTSPOT}
    if changed - allowed - {path for path in changed if is_radio(path)}:
    ...
    for marker in (b"const/16 v6, 0x3e8", b"const/16 v7, 0x63", b"const/16 v8, 0x2000"):
        if marker not in audio:
            raise RuntimeError(f"RF-loss audio buffer marker missing: {marker!r}")

    cover = method(new[SERVICE], b"public onCoverArt([B)V")
    for marker in (
        b"hasActiveNavigation",
        b"getNavigation",
        b"queueCoverArt",
        b"deferredCoverArt",
    ):
    ...
    for forbidden in (b"RemoteControlClient", b"MediaMetadata"):
        if forbidden in radio:
            raise RuntimeError(f"duplicate/metadata Bluetooth path remains: {forbidden!r}")
    for marker in (
    ...
    if b'.field public static final SSID:Ljava/lang/String; = "Citroen C3"' not in hotspot:
        raise RuntimeError("Citroen C3 hotspot regression")
    ...
    if "versionCode: 10810" not in config or "versionName: 1.8.10" not in config:
        raise RuntimeError("rebuilt APK does not report 1.8.10")

    print("verified: map frozen; 1 s audio cushion; navigation artwork fused; single Android-5 media session")
    return 0
