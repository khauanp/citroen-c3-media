#!/usr/bin/env python3
"""Apply the focused 1.8.5 map/media maintenance to the real 1.8.1 APK."""
...
"""
    ...
    # Android 5's old GPU drops the complete Path when it contains very far
    # off-screen points. Draw only segments whose two endpoints are within a
    # bounded margin around the 1280x800 viewport. This keeps every value sent
    # to drawLine small and prevents the black GPU quads seen in 1.8.4.
    const/high16 v10, -0x3c800000    # -256.0f
    ...
    const/high16 v10, 0x44c00000    # 1536.0f
    ...
    move v3, v5
"""
    value = replace_once(value, old_branch, new_branch, "bounded route segment renderer")
...
def main() -> int:
    if len(sys.argv) != 2:
        raise SystemExit("usage: apply_android_1_8_5_maintenance.py APKTOOL_DIRECTORY")
    ...
    config = replace_once(config, "  versionCode: 10801", "  versionCode: 10805", "versionCode")
    config = replace_once(config, "  versionName: 1.8.1", "  versionName: 1.8.5", "versionName")
