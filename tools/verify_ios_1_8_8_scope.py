#!/usr/bin/env python3
"""Verify C3 Link 1.8.8 routing, safety and non-fragmenting map relay against 1.8.1."""
...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit("usage: verify_ios_1_8_8_scope.py SOURCE_ZIP IOS_DIRECTORY")
    ...
    tile_relay = current[Path("C3Link/MapTileRelay.swift")].decode()
    ...
    requirements = {
        ...
        "unfiltered map tiles": "CoreImage" not in tile_relay and "CIColorControls" not in tile_relay and "let displayData = data" in tile_relay,
        "non-fragmenting tile packets": "chunkBytes = 700" in tile_relay and "2_000_000" in tile_relay,
        "bounded tile concurrency": "maxConcurrentOperationCount = 2" in tile_relay and "httpMaximumConnectionsPerHost = 2" in tile_relay,
        "validated tile dimensions": "UIImage(data: data)" in tile_relay and "tilePixels = 256" in tile_relay,
        "1.8.8 version": "MARKETING_VERSION: 1.8.8" in project and "CURRENT_PROJECT_VERSION: 11" in project,
        "1.8.8 plist": "<string>1.8.8</string>" in info and "<string>11</string>" in info,
    }
    failed = [label for label, present in requirements.items() if not present]
    if failed:
        raise RuntimeError(f"required C3 Link 1.8.8 behavior is missing: {failed}")
    ...
    print("verified: exact route; OSM safety; MTU-safe unfiltered tiles; 1.8.1 interface preserved")
    return 0
