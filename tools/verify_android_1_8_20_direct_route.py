...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit(
            "usage: verify_android_1_8_20_direct_route.py REFERENCE_1816 REBUILT_1820"
        )
    ...
    if b"AUDIO_PLAYBACK_IDLE_MS:J = 0x2ee0L" not in rebuilt[SERVICE]:
        raise RuntimeError("existing playback timer was not preserved")
    ...
    for marker in (b"NanoHTTPD", b"/set-route", b"waze_url", b"OK", b"PARAMETRO_INVALIDO"):
    ...
    for marker in (
        ...
        b"setMixedContentMode", b"WebView;->loadUrl", b"RouteReceiverServer;->start",
        b"setLayerType", b"setRenderPriority", b"setCacheMode", b"closeWazeButton",
        b"setOnClickListener", b"about:blank", b"btnFechar", b"bringToFront",
    ):
    ...
    for marker in (b"FrameLayout", b"WebView", b"wazeWebView", b"match_parent", b"closeWazeButton", b"Fechar"):
    ...
    if "versionCode: 10820" not in config or "versionName: 1.8.20" not in config:
        raise RuntimeError("rebuilt APK does not report 1.8.20")
