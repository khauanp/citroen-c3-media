...
final class MapTileRelay {
    ...
    private func fetchAndSend(_ key: MapTileKey, generation requestGeneration: Int) async {
        ...
        // CARTO Voyager remains a lightweight 2D PNG, but provides clearer
        // road hierarchy, labels and contrast than the generic OSM rendering.
        guard let url = URL(string: "https://a.basemaps.cartocdn.com/rastertiles/voyager/\(key.zoom)/\(key.x)/\(key.y).png") else {
        ...
        do {
            ...
            let partCount = Int(ceil(Double(data.count) / Double(Self.chunkBytes)))
            ...
            for part in 0..<partCount {
                ...
                let upper = min(data.count, lower + Self.chunkBytes)
                ...
                // Keep route/GPS packets responsive on the K00E while a tile
                // is split into many UDP datagrams.
